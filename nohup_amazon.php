 ...
 *Note:每天自动抓取amazon数据
 *Time:2015-1-28
 *Content:凌晨1点开始执行此NOHUP程序，扫描amazon产品数据，取当前数据
 */
...
#取产品数据
$sql="select pro_id,pro_name,pro_query_id from cate_product_amazon order by id asc";
...
while(true){
	...
	foreach($products as $k=>$product){
		...
		$pid=$product['pro_id'];
		$url="http://www.amazon.com/amazon/dp/{$product['pro_id']}";
		#$url='http://www.amazon.com/Nokia-Lumia-520-GoPhone-AT/dp/B00E45043A/ref=sr_1_1?ie=UTF8&qid=1422412837&sr=8-1&keywords=phone&pebp=1422412841022&peasin=B00E45043A#Ask';
		#提取产品评论数和6月销量数
		...
		#取浏览量数
		$rs=preg_match('/>(\d*,?\d*)\s*customer reviews/s',$data,$match);
		$orders=$rs?$match[1]:0;
		$orders=trim($orders);
		...
		$sql="insert into product_data_amazon(pro_id,date,comments,orders,query_id) values('{$pid}','{$day}','{$comments}','{$orders}','{$query_id}') on duplicate key update comments='{$comments}',orders='{$orders}'";
